...
if (/^[0-9]+$/.test(portNumberCommandLineInput) && parseInt(portNumberCommandLineInput, 10) > 0 && parseInt(portNumberCommandLineInput, 10) <= 65535) {
...
const validateIncomingMessage = (rawMessage) => {
  ...
  //   if (Object.keys(message).indexOf('wsChatKey') === -1) {
  //     result.validated = false;
...
const processSystemCommand = (message, ws, req) => {
  ...
  if (message.messageText === 'setAdmin' && adminOnLine === '') {
    ...
    console.log('message: ', message.messageText,
      ' Only one admin at a time is allowed for now. Admin is connected on:', adminOnLine);
...
const processIncomingMessage = (message, ws, req) => {
...
app.ws('/', (ws, req) => {
  ...
  ws.on('close', () => {
    ...
    if (req.headers['sec-websocket-key'] === adminOnLine) {
      adminOnLine = '';
      console.log(`соединение Админа закрыто ${req.headers['sec-websocket-key']}`);
    }
  });
});
...
app.use(express.static('dist/spa-mat/statics'));
app.use(express.static('dist/spa-mat/statics/images'));
// app.use(express.static('styles'));
// app.use(express.static('images'));
app.get('/', (req, res) => {
  res.sendFile(path.resolve(__dirname, 'index.html'));
});
const runningPort = process.env.PORT || httpServerPort;
